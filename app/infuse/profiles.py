...
def find_profiles_tfidf(algo=None):
    ...
    if not algo:
        algo = "all"
    print "computing clusters using the TF-IDF scores, using %s algo" % algo
    ...
    for username in progress(list(db.users.find().distinct('username'))):
        ...
        if algo in ["kmeans", "all"]:
            ...
            plot_2d(cluster, docs_2d, username, "kmeans", "TF-IDF")
            plot_pie(cluster, username, "kmeans", "TF-IDF")

        if algo in ["meanshift", "all"]:
            cluster = MeanShift().fit(docs_transformed) 
            print "ngrams for ms on %s" % username
            get_n_bigrams_from_clusters(cluster, docs, 3)
            plot_2d(cluster, docs_2d, username, "meanshift", "TF-IDF")
            plot_pie(cluster, username, "meanshift", "TF-IDF")
...
def find_profiles_others(algo=None):
    ...
    for username in list(db.users.find()).distinct("username"):
        ...
        if algo == "kmeans" or "all":
            ...
            plot_2d(cluster, docs_2d, username, "kmeans", "views")

        if algo == "meanshift" or "all":
            cluster = MeanShift().fit(resources) 
            plot_2d(cluster, docs_2d, username, "meanshift", "views")
...
def plot_pie(cluster, username, algo, features):
    ...
    pl.title("%s dataset clusters repartition for %s using %s" % (features, username, algo))
    fig.savefig(os.path.join(OUTPUT_PATH, 
        "figures/pie_%s_%s_%s.png" % (features, algo, username)))
...
def plot_2d(cluster, X, username, algo, features):
    ...
    fig.savefig(os.path.join(OUTPUT_PATH, 
        "figures/2d_%s_%s_%s.png" % (features, algo, username)))
